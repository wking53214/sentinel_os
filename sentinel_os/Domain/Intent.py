...
---------------------
This module defines Iceberg’s deterministic Intent Taxonomy — the canonical,
governance‑safe, replay‑friendly enumeration of caller intents used across:
...
The intent taxonomy guarantees:
...
Best‑in‑Class Notes
-------------------
- Determinism: Intent ordering is fixed and never changes.
- Governance‑Safety: Enum values are immutable and JSON‑safe.
- Replay‑Safety: Identical intent → identical routing + Bayesian behavior.
- Telemetry‑Ready: Intent labels are stable across versions.
...
class Intent(Enum):
    """
    Deterministic intent enumeration.
    ...
    def list(cls) -> List[str]:
        """
        Return canonical intent list in deterministic order.
    ...
    # ---------------------------------------------------------
    @classmethod
    def index(cls, intent: "Intent") -> int:
        """
