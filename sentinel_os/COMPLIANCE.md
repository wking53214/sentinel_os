...
- **Chain tamper-evidence is not tamper-proof at the primary alone.** `verify_chain()` catches accidental corruption and naive in-place edits. An operator with table-owner privilege can disable the ledger's protective triggers and produce a rewritten or wiped chain that still verifies clean *on the primary*. The customer-held witness twin (DAP v1, below) detects such tampering for rows already synced to a replica; setting `ICEBERG_LEDGER_RUNTIME_USER` to the restricted `ledger_reader` role closes the app-credential path specifically. `AUDIT_PLAYBOOK.md` Sections 1–3.
